...
cc.Class({
    ...
    onLoad() {
        let self = this;
        let url = cc.url.raw('resources/csv/tips.csv');
        cc.loader.load(url, function (err, res) {
            if (err) {
                ...
                //剔除标题
                console.log('[加载csv成功]------title = ' + JSON.stringify(idx_obj));
                //{"id":0,"":1,"text":2}
                data_arr.shift();
                let parse_data = [];
                ...
                console.log('[加载csv成功]------parse_data = ' + JSON.stringify(parse_data));
    ...
    on_load: function (idx_obj, parse_data) {
        ...
        for (let [key, value] of data) {
            console.log('[csv数据]------key = ' + key + ',value = ' + JSON.stringify(value));
        }
    }
