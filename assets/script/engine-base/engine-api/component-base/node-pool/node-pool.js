...
cc.Class({
    ...
    onLoad() {
        ...
        //使用对象池的一般工作流程
        //1---准备Prefab预制
        //2---初始化对象池---在场景加载的初始化脚本中，将需要数量的节点创建出来，并放进对象池
        //cc.NodePool(poolHandlerComp)---使用构造函数创建一个节点专用的对象池，该脚本需挂载在预制上
        //---start
        //参数poolHandlerComp（Function|String）---组件类型|名称，作为挂载在节点上用于处理节点回收和复用时的事件逻辑
        // this.poolHandlerComp = poolHandlerComp;
        // this._pool = []
        //---end
        //2.1使用名称（String）作为poolHandlerComp
        this.nodePool = new cc.NodePool('pool-handler-comp');
        //2.2使用构造函数（Function）作为poolHandlerComp        
        ...
        //6---清除对象池
        //clearNode---在切换场景或其它不需要对象池的时候手动调用来清空缓存节点
